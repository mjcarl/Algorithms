...
int startup(u_short *port)
{
	...
	/*建立 socket */
	//windows中AF_INET与PF_INET完全一样         //如果指定为0，表示由内核根据so_type指定默认的通信协议
	...
	if (*port == 0)  /* if dynamically allocating a port */  //如果动态分配了端口
	{
		int namelen = sizeof(name);
		if (getsockname(httpd, (struct sockaddr *)&name, &namelen) == -1)   //getsockname可以获得一个与socket相关的地址
			error_die("getsockname");
		*port = ntohs(name.sin_port);   //网络字节顺序转换为主机字节顺序   主机是从高字节到低字节的
	}
	/*开始监听*/
	if (listen(httpd, 5) < 0)   //第一个参数即为要监听的socket描述字，第二个参数为相应socket可以排队的最大连接个数
		error_die("listen");
	/*返回 socket id */
	return(httpd);
}
...
int main(void)
{
	...
	close(server_sock);    //http是基于无连接的，也就是完成一次请求后，马上断开与客户端的连接，所以一定要有这个
